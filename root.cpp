...
void Root::on_btnRoot_clicked()
{
    ...
    {
        proc->start("tools/fastboot", QStringList() << "boot" << "tools/twrp.img");
    ...
    {
        proc->start("tools/adb", QStringList() << "sideload" << "tools/magisk.zip");
    ...
    {
        proc->start("tools/adb", QStringList() << "reboot");
        ui->textBrowser->append("Root...");
